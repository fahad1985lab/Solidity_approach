...
module Complex-Model.example.reentrancy-attack.prevent-reentrancy-attack where
...
testLedger 0 .fun "withdraw" (nat Amount)  =
          ...
          λ BalanceViewFunction →
            ...
            then (exec (updatec "balance" (λ oldFun → decrementViewFunction lastcallAddr Amount oldFun)(λ oldFun oldcost msg → 1))(λ n → 1)
            λ _ →  exec (transferc Amount lastcallAddr) (λ _ → 0) λ x → return (nat 0))
