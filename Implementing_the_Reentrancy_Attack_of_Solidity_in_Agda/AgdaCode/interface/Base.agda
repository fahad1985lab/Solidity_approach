...
-- In current git repository (after 1.7.2) of agda the previous line needs to be replaced
...
module _  {I : IOInterface } (let C = Command I) (let R = Response I) where
  ...
  IOexec : ∀{i}{A : Set} (c : C) (f : R c → IO I i A) → IO I i A
  IOexec c f .force = exec' c f

  exec1 :  ∀{i} (c : C) → IO I i (R c)
  exec1 c = IOexec c return
